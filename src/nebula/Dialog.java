...
public class Dialog {
	...
	public Dialog(final Shell parent, final boolean resizable, boolean password) {
		if (parent == null) {
			shell = new Shell(Display.getCurrent(), SWT.DIALOG_TRIM | SWT.APPLICATION_MODAL | (resizable ? SWT.RESIZE : SWT.NONE));
		} else {
			shell = new Shell(parent, SWT.DIALOG_TRIM | SWT.APPLICATION_MODAL | (resizable ? SWT.RESIZE : SWT.NONE));
		...
		messageArea = password ? new PasswordMessageArea(this) : new MessageArea(this);
		footerArea = new FooterArea(this);
	}
	...
	private void center() {
		...
		if (centerPolicy == CenterOption.CENTER_ON_SCREEN || shell.getParent() == null) {
			...
			activeShell = activeShell == null ? shell : activeShell;
	...
	public static String askPassword(final String title, final String text, final String defaultValue) {
		final Dialog dialog = new Dialog(null, true, true);
		dialog.setTitle(ResourceManager.getLabel(ResourceManager.INPUT));
		MessageArea messageArea = dialog.getMessageArea();
		messageArea.setTitle(title).setText(text).setIcon(Display.getCurrent().getSystemImage(SWT.ICON_QUESTION)).addTextBox(defaultValue);
	...
	public static int comboBoxChoice(final String title, final String text, final int defaultSelection, final String... values) {
		return comboBoxChoice(null, title, text, defaultSelection, values);
	}
	
	public static int comboBoxChoice(final Shell shell, final String title, final String text, final int defaultSelection, final String... values) {
		final Dialog dialog = new Dialog(shell);
		dialog.setTitle(ResourceManager.getLabel(ResourceManager.CHOICE));
		dialog.getMessageArea().setTitle(title).setText(text).setIcon(Display.getCurrent().getSystemImage(SWT.ICON_QUESTION)).addComboButtons(defaultSelection, values);
		dialog.setButtonType(OpalDialogType.SELECT_CANCEL);
		if (dialog.show() == 0) {
			return dialog.getMessageArea().getComboChoice();
		} else {
			return -1;
		}
	}
