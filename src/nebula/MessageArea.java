...
public class MessageArea extends DialogArea {
	protected static final int INDENT_NO_ICON = 8;
	...
	protected int radioChoice;
	protected int radioDefaultSelection;
	protected String[] radioValues;

	// Informations for a exception viewer dialog box
	protected Throwable exception;
	protected Text textException;
	...
	protected int choiceDefaultSelection;
	protected ChoiceItem[] choiceValues;
	private String[] comboBoxValues;
	
	// Informations for a progress bar displayed in a dialog box
	protected ProgressBar progressBar;
	protected int progressBarMinimumValue;
	protected int progressBarMaximumValue;
	protected int progressBarValue;

	protected boolean verticalScrollbar = false;
	...
	public MessageArea addComboButtons(final int defaultSelection, final String... values) {
		setInitialised(true);
		comboBoxValues = values;
		this.comboBoxSelected = defaultSelection;
		return this;
	}
	...
	public void render() {
		...
		final boolean hasComboBox = comboBoxValues != null;
		...
		if(hasComboBox) {
			numberOfRows += comboBoxValues.length;
		}
	...
	private void createaComboBox() {
		...
		combo.setItems(comboBoxValues);
		combo.select(comboBoxSelected);
		...
		combo.addListener(SWT.Modify, new Listener() {
			@Override
			public void handleEvent(Event arg0) {
				comboBoxSelected = combo.getSelectionIndex();
			}
		});
		...
		combo.getShell().addListener(SWT.Activate, new Listener() {

			@Override
			public void handleEvent(final Event arg0) {
				combo.forceFocus();
				combo.select(comboBoxSelected);
				combo.getShell().removeListener(SWT.Activate, this);
			}
		});
	}
